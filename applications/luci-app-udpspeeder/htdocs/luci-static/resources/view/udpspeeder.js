...
return view.extend({
	load: function() {
		return Promise.all([
			L.resolveDefault(callServiceList('udpspeeder'))
		]);
	},
	render: function(res) {
		...
		m = new form.Map('udpspeeder', _('UDPspeeder') + status, _('A Tunnel which Improves your Network Quality on a High-latency Lossy Link by using Forward Error Correction,for All Traffics(TCP/UDP/ICMP)'));
