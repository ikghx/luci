...
function onclick_action(target) {
	if ( target == "webgui" ) {
		...
		fs.exec('/etc/init.d/qbittorrent-enhanced', ['start']);
		L.Poll.queue[0].fn();
	}
};
...
return L.view.extend({
	...
	render: function(v) {
		...
		o = s.taboption('connection', form.Value, 'PortRangeMin', _('Connection Port'));
		...
		o = s.taboption('webgui', form.Value, 'Locale', _('Locale Language'),
		...
		return m.render().then(function(node) {
			node.appendChild(E('script', { 'src': L.resource('view/qbittorrent-enhanced/sjcl.js') }));
	...
	handleSave: function(ev) {
		var changed, e, pwd;
		changed = document.getElementById('cbid.qbittorrent-enhanced.main.Password').getAttribute('data-changed');
		if (changed) {
			pwd = document.getElementById('widget.cbid.qbittorrent-enhanced.main.Password').value;
			e = document.getElementById('cbid.qbittorrent-enhanced.main.Password_PBKDF2');
