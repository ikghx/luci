		...
		s.addModalOptions = function(s) {
			...
			return network.getNetwork(s.section).then(L.bind(function(ifc) {
				...
				if (L.hasSystemFeature('dnsmasq') || L.hasSystemFeature('odhcpd')) {
					...
					if (protoval == 'static') {
						so = ss.taboption('general', form.Flag, 'dynamicdhcpv4', _('Dynamic <abbr title="Dynamic Host Configuration Protocol">DHCP</abbr>v4'), _('Dynamically allocate DHCPv4 addresses for clients. If disabled, only clients having static leases will be served.'));
						so.default = so.enabled;

						so = ss.taboption('general', form.Flag, 'dynamicdhcpv6', _('Dynamic <abbr title="Dynamic Host Configuration Protocol">DHCP</abbr>v6'), _('Dynamically allocate DHCPv6 addresses for clients. If disabled, only clients having static leases will be served.'));
						so.default = so.enabled;
					...
					*/
					...
					so.value('45m', _('45 minutes - default'));
					...
					so.placeholder = '90m';
					so.value('5m', _('5 minutes'));
					so.value('90m', _('90 minutes - default'));
					so.value('3h', _('3 hours'));
					so.value('12h', _('12 hours'));
					so.value('7d', _('7 days'));
					so.depends('ra', 'server');
