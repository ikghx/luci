...
return view.extend({
	...
	handleuptracker: function(m, section_id, ev) {
		return fs.exec('/etc/up_trackers.sh',
					[ '-S', section_id, '--', 'start' ])
	...
	render: function(aria2) {
		...
		if (aria2.https) {
			...
			o = s.taboption('rpc', form.Value, 'rpc_certificate', _('RPC certificate'),
				_('Use the certificate for RPC server. The certificate must be either'
			...
			o = s.taboption('rpc', form.Value, 'rpc_private_key', _('RPC private key'),
				_('Use the private key for RPC server. The private key must be'
		...
		if (aria2.https) {
			...
			o = s.taboption('http', form.Value, 'certificate', _('Certificate'),
				_('Use the certificate for HTTP server. The certificate must be either'
				+ ' in PKCS12 (.p12, .pfx) or in PEM format.'));

			o = s.taboption('http', form.Value, 'private_key', _('Private key'),
				_('Use the private key for HTTP server. The private key must be'
				+ ' decrypted and in PEM format.'));
		}
		...
		o = s.taboption('http', form.Value, 'split', _('Max number of split'),
			_('How many threads are used to download a single file.'));
		o.datatype = 'uinteger';
		o.placeholder = '5';
		...
		if (aria2.bt) {
			...
			o = s.taboption('bt', form.Flag, 'bt_load_saved_metadata', _('Load save metadata'),
				_('Before getting torrent metadata from DHT when downloading with magnet link,'
				+ ' first try to load saved torrent file.'));
			o.enabled = 'true';
			o.disabled = 'false';
			o.default = 'false';
			...
			o = s.taboption('bt', form.Flag, 'check_integrity', _('Check file integrity'),
				_('Check file integrity by validating piece hashes or a hash of entire file.'));
			o.enabled = 'true';
			o.disabled = 'false';
			o.default = 'false';

			o = s.taboption('bt', form.Flag, 'bt_detach_seed_only', _('ignore only upload task'),
				_('ignore only upload task, so that subsequent queued tasks start downloading.'));
			o.enabled = 'true';
			o.disabled = 'false';
			o.default = 'false';
			...
			o = s.taboption('bt', form.Value, 'bt_prioritize_piece', _('Try to download first and last pieces of each file first.'),
				'%s %s'.format(
					_('This is useful for previewing files.'),
					_('You can append K or M.')
				));
			...
			o = s.taboption('bt', form.Button, '_start');
			o.title      = '&#160;';
			o.inputtitle = _('Update BT Tracker');
			o.inputstyle = 'apply';
			o.onclick = L.bind(this.handleuptracker, this, m);
		}
