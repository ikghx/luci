...
function renderNeighborTable(instanceName, neighbors, hostnames) {
	var t = new L.ui.Table(
		...
		],
		{
			...
			placeholder: 'No neighbors connected'
		},
		E('em', [
	...
	t.update(neighbors.map(function(peer) {
		return [
			...
			[ +peer.throughput, '%.1mb/s'.format(+peer.throughput*1000) ],
			peer.hard_ifname,
		];
	}));
...
function renderHardifTable(instanceName, hardifs) {
	var t = new L.ui.Table(
		...
		],
		{
			...
			placeholder: 'No hardifs associated'
		},
		E('em', [
	...
	t.update(hardifs.map(function(iface) {
		return [
			...
			+iface.hop_penalty,
			+iface.elp_interval,
			+iface.throughput_override != 0 ? [+iface.throughput_override, '%.1mb/s'.format(iface.throughput_override*100000)] : [+iface.throughput_override, 'Not overridden'], //Value is reported in 100kbit/s
		];
	}));
	...
	renderIfaces: function(ifaces) {
		...
		if (res.length == 1)
			res.push(E('p', { 'class': 'center', 'style': 'margin-top:5em' }, [
				E('em', [ _('No Batman-adv interfaces configured.') ])
			]));

		}
		else {
			res.push(E('h5', { 'class': 'center', 'style': 'margin-top:5em' }, [
				E('em', [ _('This page requires either batctl-default or batctl-full to work but batctl-tiny was found.') ])
			]));
		}
