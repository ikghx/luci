...
var callServiceList = rpc.declare({
	...
	method: 'list',
	params: [ 'name' ],
	expect: { 'shairport-sync': {} }
});

return view.extend({
	load: function() {
		return Promise.all([
			L.resolveDefault(callServiceList('shairport-sync'))
		]);
	},
	render: function(res) {
		...
		m = new form.Map('shairport-sync', _('ShairPort Sync') + status, _('Shairport Sync plays audio from iTunes and AirPlay sources.'));
