...
return view.extend({
	...
	render: function () {
		...
		lhttp = ucs.taboption('general', form.DynamicList, 'listen_http', _('HTTP listeners (address:port)'));
		...
		lhttps = ucs.taboption('general', form.DynamicList, 'listen_https', _('HTTPS listener (address:port)'));
		...
		var luaPrefix = ucs.taboption('advanced', form.DynamicList, 'lua_prefix', _('Virtual path prefix for Lua scripts'));
		...
		var type = certParam.option(form.ListValue, 'key_type', _('Key type'));
		type.value('ec', _('EC'));
		type.value('rsa', _('RSA'));

		var bits = certParam.option(form.ListValue, 'bits', _('Length of key in bits'));
		bits.value('ec-256', _('ECC 256 bits'));
		bits.value('ec-384', _('ECC 384 bits'));
		bits.value('2048', _('RSA 2048 bits'));
		bits.value('3072', _('RSA 3072 bits'));
		bits.value('4096', _('RSA 4096 bits'));

		var curve = certParam.option(form.ListValue, 'ec_curve', _('Elliptic curve (EC) to use'));
		curve.depends('key_type', 'ec');
		curve.default = 'P-256';
		curve.value('P-256');
		curve.value('P-384');

		var commonname = certParam.option(form.Value, 'commonname', _('Server Hostname'));
