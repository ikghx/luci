...
if not file_path then
	HTTP.redirect(DISP.build_url("admin", "vpn", "openclash", "%s") % arg[1])
	return
end

m = Map("openclash", translate("File Edit"))
m.pageaction = false
m.redirect = DISP.build_url("admin", "vpn", "openclash", "other-file-edit", "%s") % arg[1].."?file="..HTTP.urlencode(file_path)
