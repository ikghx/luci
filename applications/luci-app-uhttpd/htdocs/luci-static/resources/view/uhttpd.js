...
return view.extend({
	...
	render: function () {
		...
		lhttp = s.taboption('general', form.DynamicList, 'listen_http', _('HTTP listeners (address:port)'));
		...
		lhttps = s.taboption('general', form.DynamicList, 'listen_https', _('HTTPS listener (address:port)'));
		...
		cert_file = s.taboption('general', form.FileUpload, 'cert', _('HTTPS Certificate (DER or PEM format)'));
		cert_file.root_directory = '/';
		cert_file.enable_remove = false;

		key_file = s.taboption('general', form.FileUpload, 'key', _('HTTPS Private Key (DER or PEM format)'));
		...
		o = s.taboption('advanced', form.DynamicList, 'lua_prefix', _('Virtual path prefix for Lua scripts'));
		o.placeholder = '/lua';
		o.optional = true;
		...
		o = s.option(form.ListValue, 'bits', _('Length of key in bits'));
		...
		o = s.option(form.ListValue, 'ec_curve', _('Elliptic curve (EC) to use'));
		o.depends('key_type', 'ec');
		o.default = 'P-256';
		o.value('P-256');
		o.value('P-384');

		o = s.option(form.Value, 'commonname', _('Server Hostname'));
