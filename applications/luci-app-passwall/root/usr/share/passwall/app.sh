...
run_chinadns_ng() {
	...
	_log_path="/dev/null"
	ln_run "$(first_type chinadns-ng)" chinadns-ng "$_log_path" -v -b 127.0.0.1 -l "${_listen_port}" ${_dns_china:+-c "${_dns_china}"} ${_dns_trust:+-t "${_dns_trust}"} ${_extra_param} ${_no_ipv6_rules:+-N=${_no_ipv6_rules}}
