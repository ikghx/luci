...
check_core_latest_version() {
	...
	if [ ! -e "$UNM_DIR/core/local_ver" ]; then
		clean_log
		echo -e "Local version: NOT FOUND, latest version: $core_latest_ver." >> "$LOG"
		update_core
	else
		if [ "$(cat $UNM_DIR/core/local_ver)" != "$core_latest_ver" ]; then
			clean_log
			echo -e "Local version: $(cat $UNM_DIR/core/local_ver 2>"/dev/null"), latest version: $core_latest_ver." >> "$LOG"
			update_core
		else
			echo -e "\nLocal version: $(cat $UNM_DIR/core/local_ver 2>"/dev/null"), latest version: $core_latest_ver." >> "$LOG"
...
update_core() {
	...
	echo -e "$core_latest_ver" > "$UNM_DIR/core/local_ver"
...
case "$1" in
	"check_version")
		if [ ! -e "$UNM_DIR/core/local_ver" ] || [ ! -e "$UNM_DIR/core/app.js" ]; then
			echo -e "Not installed."
			exit 2
		else
			version="$(node "$UNM_DIR/core/app.js" -v)"
			commit="$(cat "$UNM_DIR/core/local_ver" | head -c7)"
			echo "$version ($commit)"
			exit 0
		fi
		;;
	...
	"remove_core")
		...
		rm -rf "$UNM_DIR/core" "$UNM_DIR/core/local_ver" "$LOCK"
		;;
	*)
