...
function index()
	entry( {"admin", "vpn", "privoxy"}, cbi("privoxy"), _("Privoxy WEB proxy"), 59).acl_depends = { "luci-app-privoxy" }
	entry( {"admin", "vpn", "privoxy", "logview"}, call("logread") ).leaf = true
	entry( {"admin", "vpn", "privoxy", "startstop"}, post("startstop") ).leaf = true
	entry( {"admin", "vpn", "privoxy", "status"}, call("get_pid") ).leaf = true
end
