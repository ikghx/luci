		...
		s.addModalOptions = function(s) {
			return network.getWifiNetwork(s.section).then(function(radioNet) {
				...
				if (hwtype == 'mac80211') {
					...
					o = ss.taboption('general', form.Flag, 'acs_exclude_dfs', _('Disable DFS'), _("Do not use DFS channels on auto selection"));
					o.depends({'_freq': '5g', '_freq': 'auto', '!contains': true});
					...
					o = ss.taboption('advanced', form.Value, 'distance', _('Distance optimization'), _('Distance to the farthest network member in meters. Set only for distances above one kilometer; otherwise it is harmful.'));
				...
				o = ss.taboption('advanced', form.Value, 'mesh_rssi_threshold', _('RSSI joining threshold'), _('0 = not using RSSI threshold, 1 = do not change driver default'));
				o.rmempty = false;
				o.default = '0';
				...
				if (hwtype == 'mac80211') {
					...
					o = ss.taboption('advanced', form.Value, 'max_inactivity', _('Client inactivity limit'), _('802.11v: Inactivity limit for a client after which it will be polled or else disconnected in seconds.'));
					o.optional    = true;
					o.placeholder = 300;
					o.datatype    = 'uinteger';
				...
				o = ss.taboption('encryption', form.ListValue, 'vlan_naming', _('RADIUS VLAN Naming'), _('When hostapd creates a VLAN interface on vlan_tagged_interfaces, it needs to know how to name it.'));
				add_dependency_permutations(o, { mode: ['ap', 'ap-wds'], encryption: ['wpa', 'wpa2', 'wpa3', 'wpa3-mixed'] });
				o.value('0', _('vlan<XXX>'));
				o.value('1', _('<vlan_tagged_interface>.<XXX>'));
				o.default = '1';

				o = ss.taboption('encryption', widgets.DeviceSelect, 'vlan_tagged_interface', _('RADIUS VLAN Tagged Interface'), _('Interface where 802.1q tagged packets should appear when a RADIUS server is used.'));
				add_dependency_permutations(o, { mode: ['ap', 'ap-wds'], encryption: ['wpa', 'wpa2', 'wpa3', 'wpa3-mixed'] });
				o.size = 1;
				o.rmempty = true;
				o.multiple = false;
				...
				o = ss.taboption('encryption', form.Value, 'vlan_bridge', _('RADIUS VLAN Bridge Naming Scheme'), _('Bridge (prefix) to add the wifi and the tagged interface to.'));
				add_dependency_permutations(o, { mode: ['ap', 'ap-wds'], encryption: ['wpa', 'wpa2', 'wpa3', 'wpa3-mixed'] });
				o.rmempty = true;
				...
				if (hwtype == 'mac80211') {
					...
					o = ss.taboption('roaming', form.ListValue, 'ft_over_ds', _('Fast transition method'));
					o.depends({ ieee80211r: '1' });
					...
					o = ss.taboption('roaming', form.Flag, 'pmk_r1_push', _('PMK R1 push'), _('Enable PMK-R1 push to all R1 key holders from the R0 key holder.'));
					o.depends({ ieee80211r: '1' });
					o.placeholder = '0';
					o.rmempty = true;
					...
					if (L.hasSystemFeature('hostapd', 'eap')) {
						o = ss.taboption('roaming', form.Flag, 'ieee80211k', _('802.11k'), _('Radio Resource Measurement - Sends beacons to assist roaming. Not all clients support this.'));
				...
				if (hwtype == 'mac80211') {
					...
					o = ss.taboption('encryption', form.FileUpload, 'ca_cert2', _('CA certificate path'));
