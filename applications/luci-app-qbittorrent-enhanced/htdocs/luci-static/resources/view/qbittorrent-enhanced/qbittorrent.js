...
var CBIQBitStatus = form.DummyValue.extend({
	renderWidget: function() {
		...
		poll.add(function() {
			return callServiceList('qbittorrent-enhanced', extAgrs).then(function(res) {
				if (res.running) {
...
function onclickAction(target) {
	if ( target == "webui" ) {
		uci.load('qbittorrent-enhanced').then(function() {
			return Promise.all([
		...
		callSetInitAction('qbittorrent-enhanced', 'start').then(poll.queue[0].fn);
	}
}
