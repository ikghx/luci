...
s:option(Flag, "enable", translate("Enable App Filter"))
s.anonymous = true
...
s=m:section(TypedSection,"time",translate("时间控制"))
s.anonymous = true

hv = s:option(Value, "start_time", translate("开始时间"))
hv.default="00:00"
hv.optional=false

hv = s:option(Value, "end_time", translate("结束时间"))
hv.default="23:59"
hv.optional=false

days = s:option(MultiValue, "days")
	...
	function(meta, chunk, eof)
		...
		if eof and fd then
			...
			if ret ~= nil then
					...
					sys.exec("/etc/init.d/appfilter restart &");
