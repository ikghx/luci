...
return view.extend({
	load: function() {
		return Promise.all([
			L.resolveDefault(callServiceList('frpc'))
		]);
	},
	render: function(res) {
		...
		m = new form.Map('frpc', _('frp client') + status, _('frp is a fast reverse proxy to help you expose a local server behind a NAT or firewall to the Internet.'));
