...
s = m:section(TypedSection, "ocserv")
s.anonymous = true
...
function m.on_commit(map)
	sys.call("/usr/bin/occtl reload  >/dev/null 2>&1")
end

function e.write(self, section, value)
	if value == "0" then
		sys.call("/etc/init.d/ocserv stop >/dev/null 2>&1")
		sys.call("/etc/init.d/ocserv disable  >/dev/null 2>&1")
	else
		sys.call("/etc/init.d/ocserv enable  >/dev/null 2>&1")
		sys.call("/etc/init.d/ocserv restart  >/dev/null 2>&1")
	end
