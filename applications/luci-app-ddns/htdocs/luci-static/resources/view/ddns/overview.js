...
return view.extend({
	...
	*/
	...
	render: function(data) {
		...
		o = s.taboption('info', form.DummyValue, '_enabled', _('Status'));
		o.cfgvalue = function() {
		...
		s.addModalOptions = function(s, section_id) {
			...
			return _this.handleGetServiceData(service).then(L.bind(function (service_data) {
				...
				if (s.service_available && s.service_supported) {
					...
					o = s.taboption("advanced", form.Value, "user_agent",
						_("user agent"),
						_("user agent for wget."));
					o.placeholder = "Mozilla/5.0 (Windows NT 10.0; rv:91.0) Gecko/20100101 Firefox/95.0";
					o.optional = true;
					o.rmempty = true;
					o.modalonly = true;
					...
					o = s.taboption("timer", form.Flag, "run_once",
						_("run only once"),
						_("If no IP address change is detected, the script is only allowed to run once."));
					o.rmempty  = false;
					o.optional = true;
					o.modalonly = true;
