...
s:tab("conf", translate("server configuration"))

local file = "/etc/frp/frps.ini"
cf = s:taboption("conf", Value, "_tmp1")
cf.description = translate("This is the content of the file '/etc/frp/frps.ini'. Make changes as needed, Take effect immediately after saving & Apply.")
...
cf.write = function(self, section, value)
	...
	luci.util.exec("/etc/init.d/frps restart >/dev/null 2>&1")
end
