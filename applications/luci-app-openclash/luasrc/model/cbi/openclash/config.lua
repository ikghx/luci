...
btned.write=function(a,t)
	...
	HTTP.redirect(DISP.build_url("admin", "vpn", "openclash", "other-file-edit", "config") .. "?file=" .. HTTP.urlencode(file_path))
end
...
btnapply.write = function(self, t)
	...
	if action == "servers_manage" then
		local file_path = "/etc/openclash/config/" .. fs.basename(e[t].name)
		HTTP.redirect(DISP.build_url("admin", "vpn", "openclash", "servers") .. "?file=" .. HTTP.urlencode(file_path))
...
o.inputstyle = "reload"
o.write = function()
	HTTP.redirect(DISP.build_url("admin", "vpn", "openclash", "proxy-providers-file-manage"))
end
