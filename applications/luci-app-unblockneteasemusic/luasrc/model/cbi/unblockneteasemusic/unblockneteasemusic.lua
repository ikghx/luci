...
function enable.write(self, section, value)
	if value == "1" then
		sys.init.enable("unblockneteasemusic")
		sys.exec("/etc/init.d/unblockneteasemusic start")
	else
		sys.exec("/etc/init.d/unblockneteasemusic stop")
		sys.init.disable("unblockneteasemusic")
	end
...
daemon_enable.description = translate("开启后，附属程序会自动检测主程序运行状态，在主程序退出时自动重载")
