...
local new_text = TMP_DNSMASQ_PATH .. DNSMASQ_CONF_FILE .. DEFAULT_DNS .. LOCAL_DNS .. TUN_DNS .. REMOTE_FAKEDNS .. CHNROUTE_MODE_DEFAULT_DNS .. CHINADNS_DNS .. PROXY_MODE .. NO_PROXY_IPV6 .. new_rules
...
if not fs.access(CACHE_DNS_PATH) then
	...
	for line in io.lines("/usr/share/passwall/rules/direct_host") do
		if line ~= "" and not line:find("#") then
			...
			set_domain_ipset(line, "passwall_whitelist,passwall_whitelist6")
		end
	end
	...
	for line in io.lines("/usr/share/passwall/rules/proxy_host") do
		if line ~= "" and not line:find("#") then
			add_excluded_domain(line)
			local ipset_flag = "passwall_blacklist,passwall_blacklist6"
			if NO_PROXY_IPV6 == "1" then
				set_domain_address(line, "::")
			ipset_flag = "passwall_blacklist"
			end
		...
		uci:foreach(appname, "shunt_rules", function(s)
			...
			if _node_id ~= "nil" and _node_id ~= "_blackhole" then
				...
				if _node_id == "_direct" then
					fwd_dns = LOCAL_DNS
			ipset_flag = "passwall_whitelist,passwall_whitelist6"
				else
					fwd_dns = TUN_DNS
					ipset_flag = "passwall_shuntlist,passwall_shuntlist6"
					if NO_PROXY_IPV6 == "1" then
					ipset_flag = "passwall_shuntlist"
						no_ipv6 = true
					end
	...
	if not returnhome then
		if fs.access("/usr/share/passwall/rules/gfwlist") then
			...
			if CHNROUTE_MODE_DEFAULT_DNS == "chinadns_ng" and CHINADNS_DNS ~= "0" then
				...
				local ipset_flag = "passwall_gfwlist,passwall_gfwlist6"
				if NO_PROXY_IPV6 == "1" then
					ipset_flag = "passwall_gfwlist"
				end
				if not only_global then
					if REMOTE_FAKEDNS == "1" then
						ipset_flag = nil
		...
		if chnlist and fs.access("/usr/share/passwall/rules/chnlist") and (CHNROUTE_MODE_DEFAULT_DNS == "remote" or (CHNROUTE_MODE_DEFAULT_DNS == "chinadns_ng" and CHINADNS_DNS ~= "0")) then
			...
			if CHNROUTE_MODE_DEFAULT_DNS == "chinadns_ng" and CHINADNS_DNS ~= "0" then
				...
				for line in string.gmatch(chnlist_str, "[^\r\n]+") do
					if line ~= "" then
						set_domain_dns(line, fwd_dns)
						set_domain_ipset(line, "passwall_chnroute,passwall_chnroute6")
					end
				end
			end
		...
		if fs.access("/usr/share/passwall/rules/chnlist") then
			...
			for line in string.gmatch(chnlist_str, "[^\r\n]+") do
				if line ~= "" then
					local ipset_flag = "passwall_chnroute,passwall_chnroute6"
					if NO_PROXY_IPV6 == "1" then
					ipset_flag = "passwall_chnroute"
						set_domain_address(line, "::")
					end
					if not only_global then
						set_domain_dns(line, TUN_DNS)
